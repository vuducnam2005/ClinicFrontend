...
type Action = 'edit' | 'delete' | 'confirm' | 'cancel' | 'complete' | 'invoice' | 'pay'
...
const configs: Record<Key, Config> = {
  ...
  appointments: cfg('Quản lý lịch hẹn', 'N1 Appointment', 'Xác nhận, hủy, hoàn tất lịch hẹn và tạo hóa đơn N3.', 'GET /api/appointments', ClipboardList, cols(['id','Mã'], ['patientName','Bệnh nhân', false, false, true], ['doctorName','Bác sĩ'], ['dateTime','Ngày giờ'], ['status','Trạng thái', false, true])),
...
async function runAction(action: Action, row: Row) { if (action === 'edit') return openForm(row); actingId.value = row.id; error.value = ''; try { const id = Number(row.id); if (action === 'delete') await deleteRow(id); if (action === 'confirm') await appointmentApi.confirmAppointment(id); if (action === 'cancel') await appointmentApi.cancelAppointment(id); if (action === 'complete') await appointmentApi.completeAppointment(id); if (action === 'invoice') await billingApi.createInvoiceFromAppointment({ appointmentId: id, patientId: Number(row.patientId || 4), examFee: Number(row.feeValue || 0) }); if (action === 'pay') await billingApi.payInvoice(id); await loadData() } catch(e) { error.value = getApiErrorMessage(e) } finally { actingId.value = null } }
...
function mapPatient(x: Patient): Row { return { id: x.patientId, name: displayText(x.fullName), phone: x.phone || x.phoneNumber || 'Chưa cập nhật', gender: x.gender || 'Chưa cập nhật', history: x.medicalHistory || 'Chưa ghi nhận', raw: x } }
function mapAppointment(x: Appointment): Row { return { id: x.appointmentId, patientId: x.patientId, patientName: displayText(x.patientName), doctorName: displayText(x.doctorName), dateTime: `${date(x.appointmentDate)} · ${x.slotTime}`, status: x.status, feeValue: x.examFee, raw: x } }
function mapMedicine(x: Medicine): Row { return { id: x.medicineId, name: x.medicineName, dosageForm: x.dosageForm, price: money(x.unitPrice), stock: x.stockQuantity, stockStatus: x.stockQuantity <= 0 ? 'Hết hàng' : x.stockQuantity <= 20 ? 'Tồn thấp' : 'Đủ hàng', raw: x } }
function mapPrescription(x: MedicalRecord): Row { return { id: x.recordId || x.medicalRecordId || 'MR', patientId: x.patientId, diagnosis: x.diagnosis || 'Chưa chẩn đoán', doctorNotes: x.doctorNotes || 'Chưa ghi chú', status: 'Chờ kê đơn', raw: x } }
function mapInvoice(x: Invoice): Row { return { id: x.invoiceId, patientId: x.patientId, appointmentId: x.appointmentId ? `#${x.appointmentId}` : '-', amount: money(x.amount), status: x.status, raw: x } }
