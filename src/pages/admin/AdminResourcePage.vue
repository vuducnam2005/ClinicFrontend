...
type Action = 'edit' | 'delete' | 'confirm' | 'cancel' | 'complete' | 'pay'
...
function actions(row: Row) { const a: Array<{key: Action; label: string; className: string}> = []; const st = String(row.status || '').toLowerCase(); if (['doctors','specialties','schedules','medicines'].includes(key.value)) a.push(btn('edit','Sửa','bg-slate-100 text-slate-700 hover:bg-slate-200'), btn('delete','Xóa','bg-rose-50 text-rose-700 hover:bg-rose-100')); if (key.value === 'appointments') { if (st.includes('pending')) a.push(btn('confirm','Xác nhận','bg-teal-600 text-white hover:bg-teal-700')); if (!st.includes('cancel') && !st.includes('completed')) a.push(btn('cancel','Hủy','bg-rose-50 text-rose-700 hover:bg-rose-100')); if (st.includes('confirmed') || st.includes('inprogress')) a.push(btn('complete','Hoàn tất','bg-indigo-600 text-white hover:bg-indigo-700')) } if (key.value === 'bills' && !st.includes('paid')) a.push(btn('pay','Thu tiền','bg-teal-600 text-white hover:bg-teal-700')); return a }
function btn(key: Action, label: string, className: string) { return { key, label, className } }
async function runAction(action: Action, row: Row) { if (action === 'edit') return openForm(row); actingId.value = row.id; error.value = ''; try { const id = Number(row.invoiceId || row.id); if (action === 'delete') await deleteRow(id); if (action === 'confirm') await appointmentApi.confirmAppointment(id); if (action === 'cancel') await appointmentApi.cancelAppointment(id); if (action === 'complete') await appointmentApi.completeAppointmentSafely(id, String(row.raw?.appointmentDate || row.appointmentDate || '')); if (action === 'pay') { await billingApi.payInvoice(id, row.amountValue); note.value = 'Đã gửi yêu cầu thanh toán sang N3.' } await loadData() } catch(e) { error.value = getApiErrorMessage(e) } finally { actingId.value = null } }
