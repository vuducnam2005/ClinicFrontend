    ...
    <div class="container-page flex items-center justify-between py-4">
      ...
      <div class="hidden items-center gap-4 lg:flex">
        <RouterLink v-if="!authStore.isAuthenticated" to="/login">
          <BaseButton variant="ghost" class="rounded-2xl"><template #icon><LogIn class="h-4 w-4" /></template>&#272;&#259;ng nh&#7853;p</BaseButton>
        </RouterLink>
        <div v-else class="flex items-center gap-2">
          <RouterLink :to="dashboardRoute"><BaseButton variant="ghost" class="rounded-2xl"><template #icon><LayoutDashboard class="h-4 w-4" /></template>Dashboard</BaseButton></RouterLink>
          <BaseButton variant="ghost" class="rounded-2xl text-rose-600 hover:bg-rose-50 hover:text-rose-700" @click="handleLogout"><template #icon><LogOut class="h-4 w-4" /></template>&#272;&#259;ng xu&#7845;t</BaseButton>
        </div>
        <RouterLink to="/booking"><BaseButton class="rounded-2xl shadow-medical hover:shadow-lg hover:scale-102 active:scale-98 transition-all duration-150"><template #icon><CalendarPlus class="h-4 w-4" /></template>&#272;&#7863;t l&#7883;ch ngay</BaseButton></RouterLink>
      </div>
      ...
      <div class="flex items-center gap-3 xl:hidden">
        <button class="rounded-2xl border border-slate-200/50 p-2 text-slate-700 hover:bg-slate-100 transition" @click="open = !open">
          <Menu v-if="!open" class="h-6 w-6" /><X v-else class="h-6 w-6" />
        </button>
      </div>
    </div>
    ...
    <Transition enter-active-class="transition duration-200 ease-out" enter-from-class="-translate-y-2 opacity-0" enter-to-class="translate-y-0 opacity-100" leave-active-class="transition duration-150 ease-in" leave-from-class="translate-y-0 opacity-100" leave-to-class="-translate-y-2 opacity-0">
      <div v-if="open" class="border-t border-slate-200/70 bg-white xl:hidden">
        <div class="container-page space-y-2 py-4">
          <RouterLink v-for="item in navItems" :key="item.to" :to="item.to" class="block rounded-2xl px-4 py-2.5 text-base font-semibold text-slate-700 hover:bg-teal-50 hover:text-teal-700" @click="open = false">{{ item.label }}</RouterLink>
          <div class="grid grid-cols-2 gap-3 pt-4 border-t border-slate-100 mt-3">
            <RouterLink v-if="!authStore.isAuthenticated" to="/login" @click="open = false"><BaseButton variant="outline" class="w-full rounded-2xl">&#272;&#259;ng nh&#7853;p</BaseButton></RouterLink>
            <RouterLink v-else :to="dashboardRoute" @click="open = false"><BaseButton variant="outline" class="w-full rounded-2xl">Dashboard</BaseButton></RouterLink>
            <RouterLink to="/booking" @click="open = false"><BaseButton class="w-full rounded-2xl shadow-medical">&#272;&#7863;t l&#7883;ch</BaseButton></RouterLink>
          </div>
        </div>
      </div>
    </Transition>
...
<style scoped>
.nav-link { @apply rounded-xl px-4 py-2 text-sm font-semibold text-slate-600 transition-all duration-150 hover:bg-white hover:text-teal-700 hover:shadow-sm; }
.router-link-active:not([href='/']) { @apply bg-white text-teal-700 shadow-sm; }
